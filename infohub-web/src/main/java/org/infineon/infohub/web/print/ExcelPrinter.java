...
public class ExcelPrinter implements Printer<HSSFWorkbook> {
    ...
    private static final String[] PARTNER_PROPERTY = {"partnerNumber", "partnerName", "partnerGroup", "partnerType", "salesOrg", "region", "country", "city", "b2bManager"};
    private final static String[] CONTACT_PROPERTY = {"contactName", "contactType", "email", "telephone"};
    private static final String[] TECHNICAL_PROPERTY = {"connectionType", "techid", "businessProcess", "sourceMessage", "targetMessage", "direction", "standard", "tnFrom", "tnTo"};
    private static final String[] MESSAGE_PROPERTY = {"MSG", "EDI_MSG"};

    private static final String[] PARTNER_COLUMNS = PARTNER_PROPERTY;
    private static final String[] CONTACT_COLUMNS = Stream.concat(Arrays.stream(PARTNER_PROPERTY), Arrays.stream(CONTACT_PROPERTY)).toArray(String[]::new);
    private static final String[] TECHNICAL_COLUMNS = Stream.concat(Arrays.stream(PARTNER_PROPERTY), Arrays.stream(TECHNICAL_PROPERTY)).toArray(String[]::new);
    private static final String[] CUSTOMER_MESSAGE_COLUMNS = Stream.concat(Arrays.stream(TECHNICAL_COLUMNS), Arrays.stream(MESSAGE_PROPERTY)).toArray(String[]::new);
